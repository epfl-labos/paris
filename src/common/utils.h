...
#define ASSERT(A)
    ...
    public:
        ...
        static std::string print_map(std::unordered_map<K, V> const &m);
...
#define CHECK_BIT(var, pos) ((var) & (1<<(pos)))
    ...
    template<typename K, typename V>
    std::string Utils::print_map(std::unordered_map<K, V> const &m) {
        std::string res;
        for (auto const &pair: m) {
            res += (boost::format("Map[%s]=%s\n") % std::to_string(pair.first) % std::to_string(pair.second)).str();
        }
        return res;
    }
