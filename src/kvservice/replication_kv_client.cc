...
namespace scc {
    ...
    bool ReplicationKVClient::PushGST(PhysicalTimeSpec gst, int srcReplica) {
        PbRpcGlobalStabilizationTime pb_gst;

        pb_gst.mutable_time()->set_seconds(gst.Seconds);
        pb_gst.mutable_time()->set_nanoseconds(gst.NanoSeconds);
        pb_gst.set_srcdc(srcReplica);

        std::string serializedArg = pb_gst.SerializeAsString();

#ifdef MEASURE_STATISTICS
        SysStats::NumSentUSTs += 1;
        SysStats::NumSentUSTBytes += serializedArg.size();
#endif

        _rpcClient->Call(RPCMethod::PushGST, serializedArg);
